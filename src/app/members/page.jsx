...
const INFO_ITEMS = [
  { icon: "⏰", text: "リンクは7日間有効です" },
  { icon: "🔗", text: "リンクを受け取った人は、家族に参加できます" },
  { icon: "♾️", text: "必要に応じて何度でも生成できます" },
];
...
const CARD_CLASS =
  "backdrop-blur-2xl bg-gradient-to-br from-white/80 to-white/60 border border-[var(--gold-400)]/30 rounded-3xl overflow-hidden shadow-xl";

// ── ユーティリティ ──

function formatExpiresAt(dateString) {
...
/** ファミリー情報バッジ */
function FamilyInfoBadge({ familyName }) {
...
/** リンクセクション */
function LinkSection({ inviteUrl, copied, onCopy }) {
  return (
...
/** QRコードセクション */
function QRCodeSection({ inviteUrl, qrCodeRef, onDownload }) {
  return (
    <div className={CARD_CLASS} style={CARD_STYLE}>
