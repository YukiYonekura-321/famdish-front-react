  ...
  return (
    <div>
      ...
      {item.ingredients?.length > 0 && (
        <div className="mb-4">
          <span className="luxury-label text-base block mb-2">必要な材料</span>
...
// ── アクションボタン群 ──
function SuggestionActions({ onOk, onRetry, onNg }) {
  return (
...
export default function SuggestionCard({ suggestion, onOk, onRetry, onNg }) {
  ...
  return (
    <div className="space-y-6">
      {items.map((item, idx) => (
        <div
          ...
          className={`luxury-card max-w-3xl mx-auto animate-fade-in-up ${
            isError(item) ? "border-2 border-[var(--terracotta-300)]" : ""
          }`}
        >
          {isMultiDay && (
      ...
      <SuggestionActions onOk={onOk} onRetry={onRetry} onNg={onNg} />
    </div>
  );
}
